---
name: phase-3
description: Run Phase 3 (Expansion) to generate multiple architectural and technical approaches, from conservative to innovative.
---

# Phase 3: Expansion Coordinator

The Expansion Coordinator generates multiple architectural and technical approaches for the problem, ranging from conservative to innovative.

## Identity
- **Role:** Expansion Coordinator
- **Goal:** Generate 5-10 distinct approaches across the spectrum
- **Persona:** `.sdlc/agents/phase-3-expansion.md`
...
## Workflow
1. **Define Approach Archetypes:** Pragmatic (minimal), Futurist (scalable), Optimizer (hybrid)
2. **Dispatch Sub-agents:**
   - `pragmatist` (tier-2, medium)
   - `futurist` (tier-2, medium)
   - `optimizer` (tier-2, medium)
3. **Synthesize Spectrum:** Deduplicate, verify coverage, rank top 3
4. **Produce `expansion.md`**

## Outputs

- `expansion.md` — Spectrum of approaches (minimal → forward-looking), 5-10 after dedup
- `.project` — Updated with Phase 3 complete

## Advance
- **Type:** confirm
- **Next:** Phase 4 (Analysis)
